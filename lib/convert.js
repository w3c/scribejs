...
exports.to_markdown = (body, config, action_list) => {
     ...
     */
    function canonical_nick(nick) {
        return nick.toLocaleLowerCase().replace(/^_+/, '').replace(/_+$/, '').replace(/^@/, '');
    }
    ...
    function cleanup(body_to_clean) {
        ...
        return _.chain(split_body)
            ...
            .map(remove_preamble)
            // .map((line) => {
            //     // Number of characters to remove from the start of the line
            //     // Default is rrsagent's format
            //     let preamble = LOG_RRSAGENT;
            //     if (config.irc_format !== undefined) {
            //         switch (config.irc_format) {
            //             case 'irccloud':
            //                 preamble = LOG_IRCCLOUD;
            //                 break;
            //             case 'textual':
            //                 preamble = LOG_TEXTUAL;
            //                 break;
            //             default:
            //                 preamble = LOG_RRSAGENT;
            //         }
            //     }
            //     return line.slice(preamble);
            // })
    ...
    function generate_content_md(lines) {
        ...
        */
        const add_links = (line) => {
            ...
            // The case when the first "word" is '->' followed by a URL and a text should be treated separately
            if (words[0] === '->' && words.length >= 3 && check(words[1])) {
                const url_part = words[1];
                const link_part = words.slice(2).join(' ');
                return `See [${link_part}](${url_part})`;
            } else {
                // Handle each word separately to see if it is a link and convert them on the fly
                const converted_words = _.map(words, (word) => (check(word) ? `[${word}](${word})` : word));
        ...
        _.forEach(lines, (line_object) => {
            ...
            if (label !== null && label.toLowerCase() === 'topic') {
                ...
                within_scribed_content = false;
                add_action(content);
            // Convert the content if it is an annotated link of the sort '-> URL LINK'
