 ...
 */
async function convert(rdf_xml) {
    ...
    const js = await parser.parseStringPromise(rdf_xml);
    // Yes, this is ugly...
    // Getting to the array of rdf:li elements
    const rdf_li_list = js["rdf:RDF"]["foaf:ChatChannel"][0]["foaf:chatEventList"][0]["rdf:Seq"][0]["rdf:li"];
    // Each entry in the rdf:li is an array of foaf chatEvents, but containing only one single entry
    return rdf_li_list.map((rdf_li) => {
