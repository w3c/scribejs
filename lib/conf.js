...
*                        errors can be either not-found, or schema validation errors
*/
const json_conf_file = (fname, warn, cb) => {
    fs.readFile(fname, 'utf-8', (err, data) => {
        if (err)
            if (warn)
                cb(new Error(`No such configuration file: "${fname}"! ("${err}")`));
            else
                cb(null, {});
        else {
            ...
            const jconf = JSON.parse(data);
            console.dir(jconf);
            schemas.validate_config((validator) => {
                const valid = validator(jconf);
                if (!valid) {
                ...
                cb(_.mapObject(jconf, (value, key) => (key === 'date' ? moment(value) : value)));
            });
        }
    });
};
...
exports.get_config = () => {
    ...
    program
        ...
        .option('-s, --server', 'start HTTP server')
    ...
    if (program.server) argument_config.server = true;
    ...
    let file_config;
    if (program.config)
        json_conf_file(program.config, true, (err, data) => {
            if(err)
                file_config = {};
            else
                file_config = data;
    ...
    let user_config;
    if(process.env.HOME)
        json_conf_file(path.join(process.env.HOME, user_config_name), false, (err, data) => {
            if(err)
                user_config = {};
            else
                user_config = data;
    ...
    if (retval.torepo) {
        const needed = [retval.ghname, retval.ghemail, retval.ghtoken,
            retval.ghrepo, retval.ghpath];
...
/**
 * Interpret HTTP parameters
 *
 * @param {Object} params - parameters sent in the HTTP request
 * @returns {Object} - full configuration. See the overal manual for the field definitions
 */
exports.get_params = (params) => {
    return params; // JSON.stringify(request, null, 4);
};
