...
*/
function json_conf_file(file_name, warn) {
    let file_c = null;
    try {
        ...
        if (warn)
            throw new Error(`No such configuration file: ${file_name}!`);
        return {};
    }
...
function get_config() {
    ...
    const file_config = program.config ? json_conf_file(program.config, true) : {};
    /** ******************************************************************** */
