...
class Actions {
    ...
    async _get_issue_titles() {
        ...
        let issues = await this._repo.issues.fetch({ per_page: 200, labels: 'action' });
    ...
    add_action(id, message, name, assignee) {
        if (this._valid) {
            ...
            const full_message = this.url ? `${short_message} ([see details](${this._url}#${id}))` : short_message;
            this._actions.push({
                gh_action_id,
    ...
    async raise_action_issues() {
        if (this._actions.length !== 0) {
            console.log('Raising action issues, hold on…');
