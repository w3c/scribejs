...
//
// This library provides a more concise layer on top of the official Github API:
...
/**
 * Get a JSON file. The name provided in the
        ...
        if (name) {
            ...
            if (address !== null) {
                ...
                fetch(address)
                    ...
                    .then((body) => {
                        // Try to parse the content as JSON and, if it works, that  the final result
                        let json_content = {};
                        try {
                            json_content = JSON.parse(body);
                        } catch (err) {
                            throw new Error(`JSON parsing error in ${name}: ${err}`);
                        }
                        resolve(json_content);
                    })
                    .catch((err) => {
                        reject(new Error(`problem accessing remote file ${name}: ${err.message}`));
                    });
            } else {
                // This is a local file. Use an async function to retrieve the file,
                // a sync function would work just as well, but the result must be wrapped into a Promise anyway
                fs.readFile(name, 'utf-8', (err, body) => {
                    if (err) {
                        reject(new Error(`problem accessing local file ${name}: ${err}`));
                    } else {
                        ...
                        try {
                            ...
                            throw new Error(`JSON parsing error in ${name}: ${parse_err}`);
                        }
                        resolve(json_content);
                    }
                });
 ...
 * @returns {Promise} - a promise containing the nicknames as an object parsed from JSON.
 */
exports.get_nick_mapping = async (conf) => {
    /**
    ...
    */
    ...
    const nicks = await get_json(conf.nicknames);
 ...
 * @returns {Promise} - a promise containing the action list as an object parsed from JSON.
 */

exports.get_action_list = async (conf) => {
    const actions = await get_json(conf.actions);
    return actions;
};
 ...
 */
exports.store_action_list = async (conf, actions) => new Promise((resolve, reject) => {
    if (conf.actions) {
        try {
            // This is used just to check whether the actions were a file name or a URL
            if (check_url(conf.actions)) {
                // This was a URL, ie, shouldn't be used for save
                console.log(`Only a local file can be updated with actions (using ${conf.actions})`);
                resolve();
                return;
            }
        } catch (e) {
            // Any error has already been reported when trying to open
            // in fact, this shouldn't even happen here, just trying to be defensive
            resolve();
            return;
        }

        console.log('Writing action list');
        // If we got here, we are actually ready to save the content
        // This is a local file. Use an async function to retrieve the file,
        // though, I believe, a sync function would work just as well
