    ...
    return line_objects
        ...
        .map((line_object: LineObject): LineObject => {
            ...
            if ((line_object.nick === 'Zakim' || line_object.nick === 'zakim') && line_object.content.startsWith('agendum')) {
                ...
                const topic = line_object.content.match(agendum);
                line_object.content = `Topic: ${topic[1]}`;
                // The important point is that it should not be a nickname that is removed later;
                // because it is a topic line, the nickname will not appear in the output anyway
